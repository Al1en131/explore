...
function updateProgress(this: Draggable | void) {
  ...
  const currentX = (this && typeof this.x === 'number')
    ? this.x
...
function onProgressBarClick(e: MouseEvent) {
  ...
  const targetThumbX = Math.max(0, Math.min(maxThumbTravel, clickX - thumbPx / 2))
      ...
      <div ref="trackRef" class="gallery-track">
        ...
        >
          <div class="image-wrapper">
            <img :src="item.img" :alt="item.title" class="gallery-img" draggable="false" />
          </div>
          <div class="item-meta">
